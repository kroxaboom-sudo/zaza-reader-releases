...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/Android-13%2B-3DDC84" alt="Android 13 и новее">
...
| --- | --- |
| Обложки, поиск, категории и сортировка | Загрузки по произведениям, фильтры и управление очередью |
...
| Описание и сведения о произведении | Общий прогресс и история синхронизаций, обработка конфликтов |
...
Просмотр карточки в каталоге больше не добавляет произведение в библиотеку автоматически: используйте кнопку **Добавить в библиотеку** или импорт аккаунта. Старые записи сохранены. Скачанные произведения остаются доступны.

Список прокручивается без кнопок «Следующие 40». Поиск охватывает всю библиотеку; поиск, фильтры, сортировка и место прокрутки сохраняются при возврате из произведения. Фоновое обновление прогресса не возвращает список в начало.

Сначала синхронизируется изменённый прогресс. Недавно проверенные позиции не запрашиваются повторно в течение 10 минут, остальные проверяются частями по 100. Импорт закладок продолжается по страницам. Счётчики произведений, закладок и позиций разделены: сайт может включать записи неподдерживаемых источников.
...
Начиная с 0.6.19 требуется **Android 13 или новее**; поддержка Android 9–12 прекращена. APK один. Оба файла указывают этот минимум. Файлы `update.json` и `update-android9.json` нужны разным поколениям встроенного обновлятора. Скачивать их вручную для установки приложения не требуется. Архивы **Source code**, которые показывает GitHub, также не нужны для установки.

</details>

## Развитие проекта

Приложение активно развивается. **Preview** в названии означает тестовую сборку. Проверки проводятся на эмуляторах Android 13; это не гарантирует одинаковую работу на всех устройствах. Актуальный номер опубликованной версии показан на значке **Latest** в начале страницы.
...
Теперь экран показывает общий счётчик проверенных произведений, процент, изменения, ошибки и конфликты. Импорт списка имеет отдельный счётчик. Итоги не сбрасываются между порциями; доступны последние 10 завершённых синхронизаций.

## Управление загрузками

Главы сгруппированы по произведениям. Раскройте карточку, чтобы посмотреть главы; используйте фильтры, паузу, продолжение и повтор ошибок. При обновлении списка место прокрутки сохраняется. После удаления главы вручную или автоматически её завершённая загрузка исчезает из списка; история чтения и закладка остаются.
